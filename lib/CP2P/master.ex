defmodule CP2P.Master do
  ...
  def handle_call({:begin, %{numNodes: num_nodes, numReq: num_req}}, _from, state) do
    ...
    node_id_to_pid_map = Enum.group_by(node_id_list, & &1, & &1)
    ...
    for node_id <- node_id_list do
      ...
      Logger.debug(
        "Other node pid list: #{inspect(other_node_pid_list)} node id: #{inspect(node_id)}"
      )

      # Set other node pid list
      GenServer.call(node_info.node_pid, {:update_node_pids_in_state, other_node_pid_list})
